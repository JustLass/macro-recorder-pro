...
from PyQt5.QtWidgets import (
    ...
    QSlider, QMessageBox, QSystemTrayIcon, QMenu, QAction
)
...
arquivo_atual = [None]
timer_running = False
interromper = [False]  # Flag global de interrupção
...
class MacroRecorderApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Macro Recorder Pro (PyQt5)")
        ...
        self.btn_interromper = self.icon_button("🛑 Interromper Tudo", "#e6007a", self.interromper_tudo)
        for b in [self.btn_gravar, self.btn_parar, self.btn_reproduzir, self.btn_pausar, self.btn_carregar, self.btn_limpar, self.btn_interromper]:
            hbtns.addWidget(b)
        ...
        self.slider_vel.setMaximum(100)  # 1.0x a 10.0x
        ...
        <li><b>🛑 Interromper Tudo</b> (Ctrl+Alt+X): para qualquer reprodução imediatamente!</li>
        </ul>
        <hr>
    ...
    def criar_tray(self):
        ...
        self.tray.setToolTip("Macro Recorder Pro (PyQt5)")
        menu = QMenu()
    ...
    def reproduzir_macro(self):
        global reproduzindo, pausado, interromper
        if reproduzindo or gravando or not eventos:
            return
        reproduzindo = True
        pausado = False
        interromper[0] = False  # Sempre zera antes de rodar
        ...
        def run():
            ...
            for rep in range(reps):
                if interromper[0]: break  # <- Checa antes de cada repetição
                pressionadas = set()
                for i, e in enumerate(eventos):
                    if interromper[0]: break  # <- Checa a cada evento
                ...
                if rep<reps-1 and delay>0 and not interromper[0]:
                    QTimer.singleShot(0, lambda: self.status_label.setText(f"Aguardando {delay}s para próxima repetição..."))
                    time.sleep(delay)
            QTimer.singleShot(0, lambda: self.status_label.setText("Interrompido!" if interromper[0] else "Pronto"))
    ...
    def interromper_tudo(self):
        global interromper
        interromper[0] = True
        self.status_label.setText("Interrompendo execução...")
        self.timer_label.setText("⏱ Interrompido!")
    ...
    def atualizar_hotkeys(self):
        try:
            ...
            pass
        for key in hotkeys:
            novo = self.hotkey_inputs.get(key, None)
            if novo:
                hotkeys[key] = novo.text().strip()
        ...
        keyboard.add_hotkey(hotkeys['stop_all'], safe_thread(self.interromper_tudo))  # Hotkey para interromper tudo
